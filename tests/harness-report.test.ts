...
import { runReport, formatReport } from '../harness/report';
...
describe('runReport', () => {
  ...
  it('holds the fairness guarantee on the candidate params (invariant I-1)', () => {
    expect(runReport(OPTS).fairness.pass).toBe(true);
  });

  it('lands the delayed echo in its window on the candidate params', () => {
    expect(runReport(OPTS).echo.pass).toBe(true);
  });

  it('is honest about the candidate — does not flatter a loose juggle to a full pass', () => {
    // The candidate constants leave survival too easy and the roadmap too loose; the
    // report must say so rather than report an all-green. (Prompt 17 tightens these.)
    const report = runReport(OPTS);
